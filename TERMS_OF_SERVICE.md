# TERMS OF SERVICE

## OurCreativity Platform Terms of Service

**Effective Date:** December 11, 2025  
**Last Updated:** December 11, 2025  
**Version:** 1.0.0

---

## 📋 Agreement to Terms

By accessing or using **OurCreativity** ("the Platform", "we", "us", "our"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the Platform.

---

## 1️⃣ Definitions

- **"Platform"**: The OurCreativity website, application, and all related services
- **"User"**, **"You"**, **"Your"**: Any person or entity using the Platform
- **"Content"**: Any creative work, text, image, video, code, or other material uploaded to the Platform
- **"Account"**: Your registered user profile on the Platform
- **"Services"**: All features and functionality provided by OurCreativity

---

## 2️⃣ Eligibility

### Age Requirements

- You must be **at least 13 years old** (or 16 in the EU) to use this Platform
- If under 18, you must have parental/guardian consent
- We reserve the right to request age verification

### Account Requirements

- You must provide accurate and complete information
- You are responsible for maintaining account security
- One person or entity per account (no sharing)
- You must be a human (no bots without permission)

---

## 3️⃣ Account Registration & Security

### Creating an Account

- Provide a valid email address
- Choose a unique username
- Create a strong password
- Agree to these Terms and our Privacy Policy

### Account Security

- **You are responsible for:**
  - Keeping your password confidential
  - All activities under your account
  - Notifying us of unauthorized access

- **We are NOT responsible for:**
  - Losses due to stolen credentials
  - Unauthorized access due to your negligence

### Account Termination

- **By You:** Delete your account anytime via settings
- **By Us:** We may suspend/terminate accounts for:
  - Terms of Service violations
  - Illegal activities
  - Abuse or harassment
  - Prolonged inactivity (after notice)

---

## 4️⃣ User Content

### Content Ownership

- **You retain ownership** of content you create and upload
- You grant us a **non-exclusive, worldwide, royalty-free license** to:
  - Display your content on the Platform
  - Store and backup your content
  - Promote the Platform using your content (with attribution)

### Content Responsibilities

You are solely responsible for:

- ✅ Ensuring you have rights to upload content
- ✅ Complying with copyright and intellectual property laws
- ✅ Content accuracy and legality
- ✅ Consequences of your content

### Prohibited Content

You may NOT upload content that:

- ❌ Violates laws or regulations
- ❌ Infringes intellectual property rights
- ❌ Contains malware or malicious code
- ❌ Is hateful, discriminatory, or harassing
- ❌ Depicts violence, gore, or illegal activities
- ❌ Is sexually explicit or pornographic
- ❌ Promotes self-harm or dangerous activities
- ❌ Contains personal information of others without consent
- ❌ Is spam, scams, or fraudulent

### Content Moderation

- We reserve the right to remove content that violates these Terms
- We may (but are not obligated to) review content
- Automated systems may flag inappropriate content
- Appeals can be submitted via GitHub Issues

---

## 5️⃣ Intellectual Property

### Platform Ownership

OurCreativity owns all rights to:

- Platform design, code, and architecture
- OurCreativity name, logo, and trademarks
- "Luminous Design" philosophy and branding
- Original features and functionality

### Open Source Code

- Platform source code is licensed under **MIT License**
- See [LICENSE](./LICENSE) for code usage terms
- Trademark restrictions apply (see [TRADEMARK.md](./TRADEMARK.md))

### User-Generated Content

- Users retain copyright to their original works
- By uploading, you grant us the license described in Section 4

---

## 6️⃣ Acceptable Use Policy

### You AGREE to:

- ✅ Use the Platform for lawful purposes only
- ✅ Respect other users and their content
- ✅ Provide accurate information
- ✅ Report violations and security issues
- ✅ Comply with all applicable laws

### You AGREE NOT to:

- ❌ Impersonate others or create fake accounts
- ❌ Harass, bully, or threaten users
- ❌ Scrape, crawl, or automate access without permission
- ❌ Attempt to hack, breach, or compromise security
- ❌ Interfere with Platform operations
- ❌ Use the Platform for commercial purposes without permission
- ❌ Circumvent access controls or restrictions
- ❌ Distribute viruses or malicious software

---

## 7️⃣ Privacy & Data Protection

- Your use of the Platform is subject to our [Privacy Policy](./PRIVACY.md)
- We collect and process data as described in the Privacy Policy
- You consent to data processing by using the Platform
- See Privacy Policy for your rights and data protection measures

---

## 8️⃣ Third-Party Services

### Integrations

The Platform uses third-party services:

- **Supabase** (database and authentication)
- **Vercel** (hosting and analytics)
- See [ATTRIBUTION.md](./ATTRIBUTION.md) for full list

### Third-Party Terms

- You must comply with third-party service terms
- We are not responsible for third-party service failures
- Third-party privacy policies apply to their services

---

## 9️⃣ Disclaimers & Limitations

### Service "AS IS"

```
THE PLATFORM IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT
WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED, INCLUDING
BUT NOT LIMITED TO WARRANTIES OF MERCHANTABILITY, FITNESS FOR
A PARTICULAR PURPOSE, OR NON-INFRINGEMENT.
```

### No Guarantees

We do NOT guarantee:

- Uninterrupted or error-free service
- Complete security or data protection
- Accuracy or reliability of user content
- Compatibility with all devices/browsers
- Permanent availability of features

### Limitation of Liability

```
TO THE MAXIMUM EXTENT PERMITTED BY LAW, OURCREATIVITY SHALL
NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL,
CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED
TO LOSS OF PROFITS, DATA, USE, OR OTHER INTANGIBLE LOSSES.
```

**Maximum Liability:** Our total liability shall not exceed the amount you paid us in the past 12 months (currently $0 for free service).

---

## 🔟 Indemnification

You agree to **indemnify and hold harmless** OurCreativity, its developers, and contributors from any claims, damages, or expenses arising from:

- Your use of the Platform
- Your content or conduct
- Your violation of these Terms
- Your violation of any laws or third-party rights

---

## 1️⃣1️⃣ Changes to Terms

### Modification Rights

- We may modify these Terms at any time
- Changes will be posted with updated "Last Updated" date
- Material changes will be announced via:
  - Email notification
  - In-app announcement
  - GitHub repository notice

### Acceptance of Changes

- Continued use after changes = acceptance
- If you disagree, stop using the Platform
- Material changes effective 30 days after notice

---

## 1️⃣2️⃣ Termination

### By You

- Delete your account anytime via settings
- Data will be deleted per our [Privacy Policy](./PRIVACY.md)

### By Us

We may terminate or suspend access immediately for:

- Terms of Service violations
- Illegal activities
- Fraudulent behavior
- Extended inactivity (after 90-day notice)
- Security concerns

### Effect of Termination

- Access to Platform immediately revoked
- Content may be deleted (see Privacy Policy)
- Provisions that should survive (e.g., indemnification) remain in effect

---

## 1️⃣3️⃣ Dispute Resolution

### Governing Law

These Terms are governed by the laws of **Indonesia**, without regard to conflict of law principles.

### Dispute Process

1. **Informal Resolution:** Contact us via GitHub Issues
2. **Mediation:** Good faith attempt to resolve disputes
3. **Arbitration/Court:** As a last resort

### Jurisdiction

You agree to submit to the jurisdiction of courts in **[Specify jurisdiction]** for any disputes.

---

## 1️⃣4️⃣ Miscellaneous

### Severability

If any provision is found unenforceable, the remaining provisions remain in effect.

### Waiver

Failure to enforce any right does not waive that right.

### Assignment

- You may not assign these Terms
- We may assign to affiliates or successors

### Entire Agreement

These Terms, along with Privacy Policy and other referenced documents, constitute the entire agreement.

### Language

In case of translation conflicts, the English version prevails.

---

## 📞 Contact Us

**Questions about these Terms:**

- **GitHub Issues:** [github.com/Ardelyo/ourcreativity/issues](https://github.com/Ardelyo/ourcreativity/issues)
  - Tag: `[TERMS-QUESTION]`
- **Legal Inquiries:** Tag: `[LEGAL]`

**Reporting Violations:**

- Tag: `[VIOLATION-REPORT]`
- Include evidence and description

---

## 📚 Related Documents

- [Privacy Policy](./PRIVACY.md)
- [Trademark Policy](./TRADEMARK.md)
- [Security Policy](./SECURITY.md)
- [Code of Conduct](./CODE_OF_CONDUCT.md)
- [Contributing Guidelines](./CONTRIBUTING.md)

---

<div align="center">

**OurCreativity** - _Building a Creative Community Together_

© 2025 OurCreativity Team

**Version:** 1.0.0  
**Effective:** December 11, 2025

</div>
