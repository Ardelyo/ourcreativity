...
export const SlideBuilder: React.FC<SlideBuilderProps> = ({
  ...
  const updateSlideOrder = (dragIndex: number, hoverIndex: number) => {
    ...
    if (!dragSlide) return; // Guard against undefined
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2'>
        {slides.map((slide, index) => (
          <div
            key={slide.id}
            className='group relative flex items-center bg-[#1a1a1a] rounded-xl p-2 border border-white/5 hover:border-white/20 transition-all'
          >
              ...
              <div className='flex items-center justify-between mb-1'>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    onChange(
                      slides.map(s =>
                        s.id === slide.id ? { ...s, type: newType } : s
                      )
                    );
                  }}
                >
