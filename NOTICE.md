# NOTICE - Pemberitahuan Penting

## OurCreativity - Proyek Edukasi & Komunitas

**Versi:** 5.0.0 (Revolution Edition)  
**Copyright:** © 2025 OurCreativity Team  
**Lisensi:** MIT License (dengan pembatasan brand)

---

## 📋 Ringkasan Singkat

### Kode: Open Source (MIT License) ✅
- Gratis dipakai
- Boleh dimodifikasi
- Boleh didistribusikan
- Cocok untuk belajar dan komersial

### Brand: Dilindungi ⚠️
- Nama "OurCreativity" tidak boleh dipakai untuk project lain
- Logo dan identitas visual dilindungi
- Tujuan: Menghindari kebingungan di komunitas

**Baca detail:** [BRAND_USAGE.md](./BRAND_USAGE.md) (Bahasa Indonesia, mudah dipahami)

---

## 🎯 Aturan Utama (Simple!)

OurCreativity adalah proyek **open source untuk edukasi**, tapi ada aturan main yang perlu diikuti:

### ✅ Yang Boleh Dilakukan

```
✓ Pakai source code untuk project kamu
✓ Belajar dari kode ini
✓ Modifikasi sesuka hati
✓ Fork untuk development
✓ Kontribusi ke project ini
✓ Pakai untuk tugas kuliah/sekolah
```

### ❌ Yang TIDAK Boleh

```
✗ Pakai nama "OurCreativity" untuk website kamu
✗ Copy logo dan branding kami
✗ Klaim sebagai project OurCreativity resmi
✗ Bikin orang bingung mana yang official
```

---

## 🏛️ Hak Cipta & Kepemilikan

```
Copyright (c) 2025 OurCreativity Team
Hak Cipta Dilindungi (kecuali yang sudah dilisensikan)
```

**Tim Pencipta:**
- **Ardelyo** - Lead Developer & Designer
- **DoctorThink** - Code Refactoring & Documentation
- **OurCreativity Community** - Para Kontributor

---

## 📜 Lisensi MIT (Penjelasan Sederhana)

Kode di project ini menggunakan **MIT License**, artinya:

- **Gratis** dipakai siapa aja
- **Bebas** dimodifikasi
- **Boleh** untuk komersial
- **Harus** kasih credit/attribution

**TAPI dengan tambahan:**
- **Nama dan logo** "OurCreativity" TIDAK boleh dipakai tanpa izin

Lihat file [LICENSE](./LICENSE) untuk teks legal lengkap.

---

## 🎨 Brand yang Dilindungi

Yang **TIDAK boleh** dipakai tanpa izin:

- 🏷️ Nama "OurCreativity" (semua variasi)
- 🖼️ Logo dan ikon OurCreativity
- ✨ Filosofi "Luminous Design" (sebagai branding)
- 💬 Tagline: "Merangkai Imajinasi, Menghidupkan Karya"
- 🎨 Skema warna official (kalau dipakai sebagai identitas brand)

**Kenapa?** Biar gak bingung mana yang official, mana yang fork/modifikasi.

---

## 📦 Third-Party Libraries

Project ini pakai library open source dari pihak ketiga:

### Yang Utama:
- **React** (v19.2.0) - © Meta Platforms, Inc.
- **TypeScript** (v5.8.2) - © Microsoft Corporation
- **Vite** (v6.2.0) - © Evan You & Contributors
- **Tailwind CSS** (v3.4.17) - © Tailwind Labs, Inc.
- **Supabase** (v2.86.0) - © Supabase, Inc.

**Daftar lengkap:** Lihat [ATTRIBUTION.md](./ATTRIBUTION.md)

Semua library punya lisensi mereka sendiri (kebanyakan MIT License).

---

## 🔒 Privasi & Data

Project ini menghargai privasi:

- ✅ Tidak ngumpulin data pribadi tanpa consent
- ✅ Pakai **Supabase** untuk database (secure)
- ✅ Pakai **Vercel Analytics** (anonymous, privacy-friendly)
- ✅ Tidak ada tracking third-party
- ✅ Tidak jual data

---

## ⚠️ Disclaimer (Penafian)

```
SOFTWARE INI DIBERIKAN "APA ADANYA" TANPA JAMINAN APAPUN.
PAKAI DENGAN RISIKO SENDIRI.
```

Artinya:
- Tidak dijamin 100% bebas bug
- Kami tidak bertanggung jawab atas kerugian dari penggunaan
- Untuk edukasi dan pembelajaran

---

## 🤝 Cara Kontribusi

Mau ikut develop OurCreativity? **Silakan!**

1. Fork repository
2. Buat branch fitur baru
3. Commit perubahan
4. Buat Pull Request
5. Tunggu review

**Panduan lengkap:** [CONTRIBUTING.md](./CONTRIBUTING.md)

---

## 📞 Kontak & Pertanyaan

### Mau tanya tentang brand usage?
- **GitHub Issues:** [Link](https://github.com/ardelyo/ourcreativity/issues)
- **Tag:** `[BRAND-QUESTION]`

### Mau lapor bug atau suggest fitur?
- **GitHub Issues:** [Link](https://github.com/ardelyo/ourcreativity/issues)
- **Tag:** `[BUG]` atau `[FEATURE]`

### Pertanyaan keamanan?
- Lihat [SECURITY.md](./SECURITY.md)
- **Tag:** `[SECURITY]`

---

## 📚 Dokumentasi Lengkap

Kalau perlu info lebih detail:

| Topik | Link |
|-------|------|
| 🎨 **Brand Usage** (BACA INI!) | [BRAND_USAGE.md](./BRAND_USAGE.md) |
| 📜 Lisensi MIT | [LICENSE](./LICENSE) |
| ©️ Copyright | [COPYRIGHT.md](./COPYRIGHT.md) |
| 👥 Daftar Kontributor | [AUTHORS.md](./AUTHORS.md) |
| 📦 Third-Party Licenses | [ATTRIBUTION.md](./ATTRIBUTION.md) |
| 🛡️ Keamanan | [SECURITY.md](./SECURITY.md) |
| 🤝 Cara Kontribusi | [CONTRIBUTING.md](./CONTRIBUTING.md) |

---

## 🔄 Update Notice

Notice ini bisa diupdate sewaktu-waktu. Perubahan penting akan diumumkan via:
- GitHub release notes
- CHANGELOG.md
- Announcement di repository

---

<div align="center">

**OurCreativity** - *Merangkai Imajinasi, Menghidupkan Karya*

Dibuat dengan ❤️ dan ☕ untuk Komunitas Kreatif Indonesia

**Terima kasih sudah menghargai karya kami!** 🙏

---

© 2025 OurCreativity Team  
Proyek Edukasi & Komunitas

**Last Updated:** 11 Desember 2025

</div>
