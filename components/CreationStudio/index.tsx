...
export const CreationStudio: React.FC<Props> = ({
  isOpen,
  onClose,
  onPublish,
}) => {
  ...
  const handlePublish = () => {
    ...
    if (medium === 'visual' && formData.slides && formData.slides.length > 1)
      finalType = 'slide';
            ...
            <div className='flex items-center justify-between p-6 border-b border-white/5 bg-black/50 backdrop-blur-md z-10'>
              <div className='flex items-center gap-4'>
                {step !== 'selection' && (
                  <button
                    onClick={() =>
                      setStep(step === 'details' ? 'editor' : 'selection')
                    }
              ...
              {step === 'editor' && (
                <div className='flex gap-2'>
                  {medium === 'sinema' && (
                    <button
                      onClick={() =>
                        setSubMode(subMode === 'embed' ? 'default' : 'embed')
                      }
                    ...
                    >
                      <Globe size={12} />{' '}
                      {subMode === 'embed' ? 'Embedding Mode' : 'Embed Website'}
                    </button>
                  )}
                  {medium === 'narasi' && (
                    <button
                      onClick={() =>
                        setSubMode(
                          subMode === 'document' ? 'default' : 'document'
                        )
                      }
            ...
            <div className='flex-1 overflow-hidden relative flex'>
              ...
              {step === 'editor' && (
                <div className='w-full h-full flex flex-col md:flex-row'>
                  {/* Main Editor Surface */}
                  <div
                    className={`flex-1 p-4 relative flex flex-col ${medium === 'kode' ? 'w-full md:w-1/2 h-1/2 md:h-full border-b md:border-b-0 md:border-r border-white/5' : 'h-full'}`}
                  >
                    {/* --- KODE EDITOR --- */}
                    {medium === 'kode' && (
                      <CodeEditor
                  ...
                  {medium === 'kode' && (
                    <div className='w-full md:w-1/2 h-1/2 md:h-full p-4 bg-[#050505] flex flex-col'>
                      <div className='text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider flex justify-between'>
                        ...
                        <PyodideSandbox
                          code={
                            formData.code_language === 'python'
                              ? formData.content || ''
                              : ''
                          }
                          triggerRun={
                            formData.code_language === 'python' ? triggerRun : 0
                          }
