  ...
  return (
    <div className='fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 px-4 pointer-events-none'>
      <motion.nav
        ...
        className={`pointer-events-auto bg-[#111]/90 backdrop-blur-lg border border-white/10 shadow-2xl shadow-black/50 overflow-hidden flex flex-col ${
          isMobileMenuOpen
            ? 'p-6 gap-6'
            : showFullMenu
              ? 'px-6 py-3'
      ...
      >
        <motion.div
          layout
          className={`flex items-center justify-between w-full ${isMobileMenuOpen ? '' : showFullMenu ? 'gap-8' : 'gap-2'}`}
        >
          {/* Logo */}
          <Link
            to='/'
                ...
                >
                  {navLinks.map(link => (
                    <Link
                      key={link.name}
                      to={link.href}
                      className={`px-4 py-2 text-xs font-medium uppercase tracking-wide transition-colors rounded-full whitespace-nowrap ${
                        isActive(link.href)
            ...
            >
              ...
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    to={link.href}
                    className={`text-lg font-medium px-4 py-3 rounded-xl transition-colors flex items-center justify-between group ${
                      isActive(link.href)
                  ...
                  >
                    ...
                    <ArrowRight
                      size={16}
                      className={`opacity-0 group-hover:opacity-100 transition-opacity ${isActive(link.href) ? 'opacity-100' : ''}`}
                    />
                  </Link>
                </motion.div>
              ))}
