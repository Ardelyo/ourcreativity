...
export const Announcement = () => {
  const [activeTab, setActiveTab] = useState<'updates' | 'changelog'>(
    'updates'
  );
      ...
      <div className='pt-24 md:pt-32 pb-12 px-4 text-center relative overflow-hidden'>
        ...
        >
          ...
          <p className='text-gray-400 max-w-2xl mx-auto text-base md:text-lg mb-10'>
            Ikuti perkembangan terbaru dari komunitas dan evolusi teknis
            platform kami.
          </p>
          ...
          >
            ...
            {events.length > 0 && (
              ...
              >
                ...
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${events[0].color} opacity-20 group-hover:opacity-30 transition-opacity duration-700`}
                />
                <div
                  className={`absolute -right-[20%] -top-[20%] w-[80%] h-[80%] bg-gradient-to-bl ${events[0].color} rounded-full blur-[150px] opacity-20 group-hover:opacity-40 transition-opacity duration-700 animate-pulse`}
                />
                ...
                <div className='relative z-10 h-full flex flex-col md:flex-row items-center p-8 md:p-16 gap-12'>
                  <div className='flex-1 space-y-8'>
                    ...
                    <div className='space-y-4'>
                      <motion.h2
                        layoutId={`title-${events[0].id}`}
                        className='text-5xl md:text-7xl font-serif font-bold text-white leading-[1.1]'
                      >
                        {events[0].title}
                      </motion.h2>
                      <p
                        className={`text-xl md:text-2xl font-medium text-transparent bg-clip-text bg-gradient-to-r ${events[0].color}`}
                      >
                        {events[0].subtitle}
                      </p>
                    </div>
                  ...
                  <div className='w-full md:w-1/3 aspect-square relative hidden md:block'>
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${events[0].color} rounded-full blur-3xl opacity-20 animate-pulse`}
                    />
                ...
                >
                  ...
                  <div className='p-8 relative z-10 flex flex-col h-full'>
                    <div className='flex justify-between items-start mb-6'>
                      <span
                        className={`text-xs font-bold px-3 py-1 rounded-full border border-white/10 ${event.status === 'Baru' ? 'bg-rose-500/20 text-rose-400' : 'bg-gray-800 text-gray-400'}`}
                      >
                        {event.status}
                      </span>
                    ...
                    <motion.h3
                      ...
                      className='font-serif text-2xl text-white mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-400 transition-all'
                    >
                      {event.title}
                    </motion.h3>
              ...
              <div className='min-h-full flex items-center justify-center p-4 md:p-8'>
                ...
                >
                  ...
                  <div className={`h-64 md:h-80 relative overflow-hidden`}>
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${selectedEvent.color} opacity-20`}
                    />
                    <div className='absolute inset-0 flex items-center justify-center'>
                      <div
                        className={`w-96 h-96 rounded-full bg-gradient-to-r ${selectedEvent.color} blur-[100px] opacity-40 animate-pulse`}
                      />
                    </div>
                    ...
                    <div className='flex flex-wrap gap-3 pt-4'>
                      {selectedEvent.highlights?.map(
                        (tag: string, idx: number) => (
                          <span
                            key={idx}
                            className='px-4 py-2 rounded-full bg-white/5 border border-white/10 text-sm font-bold text-gray-300'
                          >
                            # {tag}
                          </span>
                        )
