...
const GlitchText = ({ text, active }: { text: string; active: boolean }) => {
  return (
    <div className='relative inline-block'>
      ...
      {active && (
        <>
          <span className='absolute top-0 left-0 -ml-1 text-red-500 opacity-70 animate-pulse'>
            {text}
          </span>
          <span className='absolute top-0 left-0 ml-1 text-blue-500 opacity-70 animate-pulse delay-75'>
            {text}
          </span>
        </>
      )}
...
export const VideoPage = () => {
  ...
  // Adegan 3: Gaya
  const stylesOpacity = useTransform(
    scrollYProgress,
    [0.45, 0.5, 0.7, 0.75],
    [0, 1, 1, 0]
  );
  const stylesX = useTransform(
    scrollYProgress,
    [0.45, 0.75],
    ['100%', '-100%']
  );
  ...
  const alchemyOpacity = useTransform(
    scrollYProgress,
    [0.7, 0.75, 0.85, 0.9],
    [0, 1, 1, 0]
  );
  ...
  return (
    <div
      ref={containerRef}
      className='h-[600vh] bg-[#050505] text-white font-sans selection:bg-orange-500 selection:text-black relative'
    >
      ...
      <div className='fixed inset-0 pointer-events-none z-50 flex flex-col justify-between p-6 md:p-12'>
        ...
        <div className='flex justify-between items-start'>
          <div className='pointer-events-auto'>
            <Link
              to='/info'
              className='flex items-center gap-2 text-xs font-mono text-gray-500 hover:text-orange-500 transition-colors uppercase tracking-widest border border-gray-800 px-4 py-2 rounded bg-black/80 backdrop-blur-md'
            >
        ...
        <div className='w-full bg-black/80 backdrop-blur-sm p-4 rounded-xl border border-gray-800/50 pointer-events-auto'>
          ...
          <div className='flex justify-center gap-8 mb-4 text-gray-500'>
            ...
            <Play
              size={20}
              className='text-orange-500 fill-current hover:scale-110 transition-transform cursor-pointer'
            />
            <SkipForward
              size={20}
              className='hover:text-white cursor-pointer transition-colors'
          ...
          <div className='relative h-12 bg-[#111] border border-gray-800 rounded overflow-hidden flex items-center cursor-pointer group'>
            ...
            <div className='absolute top-0 left-0 right-0 h-2 flex justify-between px-2 opacity-30'>
              {Array.from({ length: 50 }).map((_, i) => (
                <div
                  key={i}
                  className={`w-[1px] bg-gray-500 ${i % 5 === 0 ? 'h-2' : 'h-1'}`}
                ></div>
              ))}
            </div>
            ...
            >
              <div className='absolute top-0 -translate-x-1/2 -mt-1 text-orange-500 text-[10px]'>
                ▼
              </div>
            </motion.div>
          </div>
      ...
      <div className='fixed inset-0 w-full h-full overflow-hidden flex items-center justify-center pointer-events-none z-0 pb-40'>
        ...
        <motion.div
          ...
          className='absolute inset-0 flex flex-col items-center justify-center bg-[#050505] z-40 pb-40'
        >
          ...
          <p className='font-mono text-orange-500 text-sm tracking-[0.5em] animate-pulse mt-4'>
            GULIR UNTUK MEMUTAR
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className='absolute inset-0 flex flex-col items-center justify-center z-30 pb-40'
        >
          ...
          <div className='flex flex-wrap justify-center gap-4 mt-8'>
            {scenes[1]?.tags?.map((tag, i) => (
              <span
                key={i}
                className='bg-orange-500 text-black font-bold px-4 py-1 text-sm md:text-xl transform rotate-[-5deg] hover:rotate-0 transition-transform'
              >
                #{tag}
              </span>
            ))}
        ...
        <motion.div
          style={{ opacity: stylesOpacity, x: stylesX }}
          className='absolute flex items-center gap-20 pl-[50vw] z-20 whitespace-nowrap pb-40'
        >
          {scenes[2]?.items?.map((item, i) => (
            <div
              key={i}
              className={`text-[15vw] font-black ${item.color} opacity-80 hover:opacity-100 transition-opacity drop-shadow-2xl`}
            >
              {item.label}
            </div>
          ))}
        ...
        <motion.div
          style={{ opacity: alchemyOpacity, scale: alchemyScale }}
          className='absolute inset-0 flex items-center justify-center z-30 bg-[#0a0a0a] pb-40'
        >
          <div className='relative w-full max-w-4xl p-8'>
            ...
            <div className='relative z-10 text-center'>
              ...
              >
                ...
                <h3 className='text-3xl font-bold text-orange-500 tracking-widest uppercase'>
                  Color Grading
                </h3>
              </motion.div>
              ...
              <motion.div
                style={{ y: soundY, opacity: soundOpacity }}
                className='bg-gray-900/80 backdrop-blur border border-gray-800 p-8 rounded-2xl'
              >
                ...
                <p className='text-xl text-gray-300 font-mono'>
                  "Kami memberi warna pada abu-abu.
                  <br />
                  Kami memberi suara pada keheningan."
                </p>
              </motion.div>
            </div>
          </div>
        ...
        <motion.div
          ...
          className='absolute inset-0 flex flex-col items-center justify-center z-20 bg-[#050505] pointer-events-auto pb-40'
        >
