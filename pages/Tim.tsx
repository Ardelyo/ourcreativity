...
const generatePersona = (login: string, isOwner: boolean): CreativePersona => {
  if (isOwner) {
    return {
      ...
      description:
        'Arsitek mimpi yang merajut benang pertama dari tapestri digital ini. Cahaya penuntun bagi kolektif.',
      adjective: 'Bercahaya',
      element: 'Aether',
    };
  }
  ...
  return {
    title: titles[charSum % titles.length] || 'Kreator Digital',
    description:
      descriptions[charSum % descriptions.length] || 'Kontributor aktif.',
    adjective: adjectives[charSum % adjectives.length] || 'Kreatif',
    element: elements[charSum % elements.length] || 'Digital',
  };
};
...
export const Tim = () => {
  ...
  const [selectedContributor, setSelectedContributor] =
    useState<EnhancedContributor | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const enhancedContributors: EnhancedContributor[] = contribData.map(
          c => {
            const stat = statsData.find(
              s => s.author?.login?.toLowerCase() === c.login.toLowerCase()
            );
            const totalAdditions =
              stat?.weeks.reduce((sum, w) => sum + w.a, 0) || 0;
            const totalDeletions =
              stat?.weeks.reduce((sum, w) => sum + w.d, 0) || 0;
        ...
        if (cached) {
          ...
          const fallbackEnhanced: EnhancedContributor[] =
            FALLBACK_CONTRIBUTORS.map(c => {
              const isOwner =
                c.login.toLowerCase() === OWNER_LOGIN.toLowerCase();
              return {
                ...c,
                totalAdditions: 0,
                totalDeletions: 0,
  ...
  const contributorCards = useMemo(() => {
    return contributors.map(contributor => {
      const isOwner =
        contributor.login.toLowerCase() === OWNER_LOGIN.toLowerCase();
      return (
        <ContributorCard
    ...
    <div className='pt-36 pb-20 px-4 max-w-7xl mx-auto min-h-screen relative z-10'>
      ...
      >
        ...
        <p className='text-white/40 max-w-xl mx-auto text-lg font-light leading-relaxed'>
          Konstelasi pemikiran yang merajut masa depan digital. Setiap commit
          adalah benang, setiap fitur adalah kenangan.
        </p>
      </motion.div>
      ...
      {loading && (
        ...
        >
          <div className='relative w-16 h-16'>
            ...
            <div
              className='absolute inset-2 border-t-2 border-l-2 border-purple-500 rounded-full animate-spin'
              style={{ animationDirection: 'reverse' }}
            />
          </div>
          <p className='font-serif italic text-white/30 tracking-widest text-sm'>
            Memanggil roh-roh kreatif...
          </p>
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {selectedContributor && (
          <ContributorModal
            ...
            isOwner={
              selectedContributor.login.toLowerCase() ===
              OWNER_LOGIN.toLowerCase()
            }
          />
        )}
      </AnimatePresence>
