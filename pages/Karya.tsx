...
export const Karya = () => {
  ...
  const filteredArtworks =
    filter === 'all' ? artworks : artworks.filter(a => a.division === filter);
        ...
        return (
          <div className='w-full h-full bg-[#f0f0f0] text-black p-8 flex flex-col justify-center items-center text-center font-serif relative overflow-hidden group-hover:bg-white transition-colors'>
            <AlignLeft
              ...
              size={24}
            />
            <p className='text-lg italic leading-relaxed line-clamp-6'>
              "{art.content}"
            </p>
      ...
      case 'code':
        return (
          <div className='w-full h-full bg-[#0d1117] p-6 font-mono text-xs text-gray-300 relative overflow-hidden group-hover:bg-[#161b22] transition-colors'>
            ...
            <Code
              className='absolute bottom-4 right-4 text-green-500/50'
              size={24}
            />
          </div>
        );
      ...
      <div className='flex flex-col md:flex-row justify-between items-end mb-12 gap-6'>
        <div>
          ...
          <p className='text-gray-400 text-lg'>
            Kurasi visual terbaik dari komunitas.
          </p>
        </div>
        ...
        <div className='flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar'>
          <div className='flex bg-[#111] p-1 rounded-full border border-white/10'>
            {['all', 'graphics', 'video', 'writing', 'coding', 'meme'].map(
              f => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
      ...
      {loading ? (
        <div className='columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6'>
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className='break-inside-avoid bg-[#111] rounded-3xl overflow-hidden mb-6 h-64 animate-pulse border border-white/5'
            >
        ...
        <div className='columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6'>
          {filteredArtworks.map((art, index) => (
            ...
            >
              ...
              <div
                className={`relative w-full ${art.type === 'text' || art.type === 'code' ? 'aspect-[4/5]' : ''}`}
              >
                <motion.div
                  layoutId={`content-${art.id}`}
                ...
                <div className='absolute inset-0 bg-black/60 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-between p-6'>
                  <div className='flex justify-between items-start transform -translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-75'>
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border border-white/20 ${
                        art.division === 'coding'
                  ...
                  <div className='transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-100'>
                    ...
                    <div className='flex items-center gap-2 mb-4'>
                      <div className='w-5 h-5 rounded-full bg-gray-700 overflow-hidden'>
                        <img
                          src={`https://ui-avatars.com/api/?name=${art.author}&background=random`}
                          alt='Avatar'
                        />
                      </div>
        ...
        <div className='flex flex-col items-center justify-center py-20 text-center'>
          ...
          <p className='text-gray-400 max-w-md'>
            Kategori ini masih kosong. Jadilah yang pertama mempublikasikan
            karya di sini!
          </p>
          <button
            onClick={() => setIsStudioOpen(true)}
                    ...
                    <div className='w-full h-full bg-black relative'>
                      <iframe
                        srcDoc={generateCodePreview(
                          selectedArtwork.content,
                          selectedArtwork.code_language || 'html'
                        )}
              ...
              <div className='w-full md:w-2/5 p-8 md:p-12 overflow-y-auto custom-scrollbar bg-[#111] border-l border-white/10'>
                <div className='flex items-center justify-between mb-8'>
                  <div
                    className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border border-white/10 ${
                      selectedArtwork.division === 'coding'
                        ? 'bg-green-500/10 text-green-400'
                        : selectedArtwork.division === 'video'
                          ? 'bg-orange-500/10 text-orange-400'
                          : selectedArtwork.division === 'meme'
                            ? 'bg-yellow-500/10 text-yellow-400'
                            : selectedArtwork.division === 'writing'
                ...
                <div className='flex items-center gap-4 mb-8 p-4 bg-white/5 rounded-2xl border border-white/5'>
                  <div className='w-12 h-12 rounded-full bg-gray-700 overflow-hidden'>
                    <img
                      src={`https://ui-avatars.com/api/?name=${selectedArtwork.author}&background=random`}
                      alt='Avatar'
                    />
                  </div>
                  <div>
                ...
                <div className='flex flex-wrap gap-2 mb-8'>
                  {selectedArtwork.tags?.map((tag: string) => (
                    <span
                      key={tag}
                      className='px-3 py-1 rounded-full border border-white/10 text-xs text-gray-400 hover:text-white hover:border-white transition-colors cursor-pointer'
                    >
                      #{tag}
                    </span>
                  ))}
