...
const projects = [
  {
    ...
    category: 'Web App',
    status: 'Deployed',
    desc: 'Platform komunitas next-gen dengan React & Node.js',
  },
  {
    title: 'CyberGuard',
    category: 'Keamanan',
    status: 'Aktif',
    desc: 'Scanner kerentanan otomatis untuk jaringan lokal',
  },
  {
    title: 'AlgoVisualizer',
    category: 'Edukasi',
    status: 'Beta',
    desc: 'Visualisasi algoritma sorting interaktif',
  },
  {
    title: 'Bot: Sentinel',
    category: 'Otomasi',
    status: 'Online',
    desc: 'Bot moderasi Discord dengan kapabilitas AI',
  },
];
  ...
  return (
    ...
    >
      ...
      <div className='fixed top-8 left-8 z-50'>
        <Link
          to='/info'
          className='inline-flex items-center text-green-500 hover:text-green-400 transition-colors group bg-black/80 px-4 py-2 border border-green-500/30 rounded-sm'
        >
          <ArrowLeft
            size={16}
        ...
        {projects.map((project, i) => (
          ...
          >
            ...
            <div className='flex justify-between items-start mb-6 border-b border-green-500/20 pb-4'>
              ...
              <span
                className={`text-xs px-2 py-1 rounded-sm ${project.status === 'Deployed' || project.status === 'Online' || project.status === 'Aktif' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}
              >
                {project.status}
              </span>
            </div>
            ...
            <span className='text-xs text-green-500 mb-4 block'>
              [{project.category}]
            </span>
        ...
        <div className='max-w-6xl w-full grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10'>
          {[
            ...
            >
              ...
              <div className='text-sm font-bold text-green-500/70 uppercase tracking-widest'>
                {stat.label}
              </div>
            </motion.div>
          ))}
