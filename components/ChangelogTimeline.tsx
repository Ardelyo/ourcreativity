          ...
          return (
            <div key={majorVer} className='relative'>
              ...
              <div className='sticky top-24 z-30 flex items-center gap-4 mb-8 pl-2'>
                {/* Era Badge */}
                <div
                  className={`relative z-10 w-14 h-14 md:w-18 md:h-18 rounded-full bg-[#0a0a0a] border-4 border-[#0a0a0a] flex items-center justify-center ${eraTheme.glow}`}
                >
                  <div
                    className={`w-full h-full rounded-full bg-gradient-to-br ${eraTheme.gradient} flex items-center justify-center`}
                  >
                    <span className='font-black text-white text-base md:text-xl drop-shadow-lg'>
                      v{majorVer}
                    </span>
                  </div>
                </div>
                {/* Era Name Pill */}
                <div
                  className={`bg-[#0a0a0a]/90 border ${eraTheme.borderColor} px-5 py-2.5 rounded-full backdrop-blur-md shadow-lg`}
                >
                  <span
                    className={`font-serif text-lg md:text-xl font-bold bg-gradient-to-r ${eraTheme.gradient} bg-clip-text text-transparent`}
                  >
                    {eraTheme.name}
                  </span>
                </div>
              ...
              <div className='space-y-6 pl-12 md:pl-20'>
                {groupedChangelogs[majorVer]?.map((entry, index) => {
                  ...
                  const isMajorVersion =
                    entry.version.endsWith('.0') &&
                    !entry.version.includes('.0.');
                  const isPatch =
                    entry.version.includes('.0.') || entry.category === 'Patch';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`absolute -left-[2.5rem] md:-left-[3.5rem] top-8 w-6 md:w-10 h-px bg-gradient-to-r ${eraTheme.gradient} opacity-30 group-hover:opacity-60 transition-opacity`}
                      />

                      {/* Connector Node - Era Themed */}
                      <div
                        className={`absolute -left-[3rem] md:-left-[4rem] top-5 w-7 h-7 rounded-full bg-[#0a0a0a] border-2 ${isExpanded ? eraTheme.borderColor : 'border-white/20'} ring-4 ring-[#0a0a0a] flex items-center justify-center z-10 transition-all duration-300 group-hover:scale-110 ${isExpanded ? eraTheme.glow : ''}`}
                      >
                        <div
                          className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${isExpanded ? `bg-gradient-to-br ${eraTheme.gradient}` : 'bg-white/30 group-hover:bg-white/60'}`}
                        />
                      </div>
                      ...
                      >
                        ...
                        <div
                          className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl ${entry.color || eraTheme.gradient} opacity-5 rounded-bl-full pointer-events-none`}
                        />
                        ...
                        {isExpanded && (
                          <>
                            <div
                              className={`absolute inset-0 bg-gradient-to-br ${entry.color || eraTheme.gradient} opacity-[0.03] pointer-events-none`}
                            />
                            <div
                              className={`absolute -top-20 -right-20 w-40 h-40 bg-gradient-to-br ${entry.color || eraTheme.gradient} opacity-10 blur-3xl pointer-events-none`}
                            />
                          </>
                        )}
                        ...
                        {isMajorVersion && (
                          <div
                            className={`absolute top-3 right-3 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest bg-gradient-to-r ${entry.color || eraTheme.gradient} text-white shadow-lg`}
                          >
                            ★ MAJOR
                          </div>
                        )}
                          ...
                          <div className='flex justify-between items-start gap-4'>
                            <div className='space-y-2 flex-1'>
                              ...
                              <div className='flex flex-wrap items-center gap-2 mb-2'>
                                <span
                                  className={`px-2.5 py-1 rounded-md text-xs font-bold tracking-wide border ${entry.color ? `bg-gradient-to-r ${entry.color} text-white border-transparent` : 'bg-white/10 text-gray-300 border-white/10'}`}
                                >
                                  {entry.version}
                                </span>
                              ...
                              {/* Title */}
                              <h3
                                className={`font-serif text-xl md:text-2xl font-bold leading-tight transition-colors ${isExpanded ? 'text-white' : 'text-gray-200 group-hover:text-white'}`}
                              >
                                {entry.title}
                              </h3>
                              {/* Description Preview */}
                              ...
                              >
                                <div className='pt-6 space-y-5'>
                                  ...
                                  <div className='relative pl-4 border-l-2 border-white/10'>
                                    <div className='absolute -left-2 -top-1 text-2xl text-white/20 font-serif'>
                                      "
                                    </div>
                                      ...
                                      <div className='bg-black/40 rounded-xl p-4 border border-white/5'>
                                        ...
                                        <ul className='space-y-2'>
                                          {entry.changes.map((change, idx) => (
                                            ...
                                            >
                                              <span
                                                className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 bg-gradient-to-r ${entry.color || eraTheme.gradient}`}
                                              />
